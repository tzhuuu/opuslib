...
_get_size = opuslib.constants.libopus.opus_encoder_get_size
...
_create = opuslib.constants.libopus.opus_encoder_create
_create.argtypes = (
    ctypes.c_int, ctypes.c_int, ctypes.c_int, opuslib.constants.c_int_pointer)
...
def create(fs, channels, application):
    ...
    if result_code.value is not opuslib.constants.constants.OK:
        raise opuslib.exceptions.OpusError(result_code.value)

    return result


_ctl = opuslib.constants.libopus.opus_encoder_ctl
...
_encode = opuslib.constants.libopus.opus_encode
_encode.argtypes = (
    EncoderPointer, opuslib.constants.c_int16_pointer, ctypes.c_int,
...
def encode(encoder, pcm, frame_size, max_data_bytes):
    ...
    pcm = ctypes.cast(pcm, opuslib.constants.c_int16_pointer)
...
_encode_float = opuslib.constants.libopus.opus_encode_float
_encode_float.argtypes = (
    EncoderPointer, opuslib.constants.c_float_pointer, ctypes.c_int,
    ctypes.c_char_p, ctypes.c_int32)
...
def encode_float(encoder, pcm, frame_size, max_data_bytes):
    ...
    pcm = ctypes.cast(pcm, opuslib.constants.c_float_pointer)
    data = (ctypes.c_char * max_data_bytes)()
...
destroy = opuslib.constants.libopus.opus_encoder_destroy
